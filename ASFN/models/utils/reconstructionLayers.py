...
class ResidualBlock_noBN(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            x: with shape of [b, c, h, w]

        Returns: processed features with shape [b, c, h, w]
...
class ResidualBlock_11Conv_noBN(nn.Module):
    """Residual block w/o BN
    ---Conv-ReLU-Conv-+-
     |________________|
    """
    ...
    def __init__(self, nf=64):
        super(ResidualBlock_11Conv_noBN, self).__init__()
        self.conv1 = nn.Conv2d(nf, nf, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv2 = nn.Conv2d(nf, nf, kernel_size=3, stride=1, padding=1, bias=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)
        self.mod = nn.Conv2d(nf, nf, kernel_size=1, stride=1, padding=0, bias=True)
