...
class VanillaDeconv(nn.Module):
    ...
    def forward(self, xs):
        xs_resized = F.interpolate(xs, scale_factor=self.scale_factor, mode='bilinear', align_corners=False)
...
class PartialDeconv(nn.Module):
    ...
    def forward(self, input_tuple):
        inp, mask = input_tuple
        inp_resized = F.interpolate(inp, scale_factor=self.scale_factor, mode='bilinear', align_corners=False)
        with torch.no_grad():
